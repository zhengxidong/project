<?php /* Smarty version Smarty-3.1.14, created on 2017-03-29 06:58:48
         compiled from ".\template\User\user_resetoldpassword.html" */ ?>
<?php /*%%SmartyHeaderCode:1565758da812f2d0a62-22977631%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    'eb3ffb7299efbd2f71f5a1feca050e3fc7b7d1ac' => 
    array (
      0 => '.\\template\\User\\user_resetoldpassword.html',
      1 => 1490768885,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '1565758da812f2d0a62-22977631',
  'function' => 
  array (
  ),
  'version' => 'Smarty-3.1.14',
  'unifunc' => 'content_58da812f391979_90308124',
  'variables' => 
  array (
    'arr' => 0,
  ),
...
<?php if ($_valid && !is_callable('content_58da812f391979_90308124')) {function content_58da812f391979_90308124($_smarty_tpl) {?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
            ...
            <div class="user_Account_style">
                ...
                <div id="J_main" class="m-content">
                    ...
                    <div class="m-content-hd">
                        <div class="m-page-tab"><a href="?c=user&a=reset" class="tab-item" data-track="done">手机验证</a><a href="?c=user&a=resetoldpassword" class="tab-item z-current" data-track="done">原登录密码验证</a></div>
                    </div>
                    <!-- 原登录密码验证——步骤一 -->
                    <div id="J_passwordVerify" class="m-content-bd f-hidden">

                        <div class="m-verification-form J_step f-hidden">
                            <div class="m-vf-item">
                                <label class="ui-label u-label">旧密码：</label>
                                <div class="ui-form-item-group">
                                    <input name="jiupassword" maxlength="20" id="J_password_oldpwd" name="verification-new-pwd" class="ui-input u-verification-input" placeholder="请输入原登录密码" type="password">
                                    <div id="J_password_oldpwd_err" style="display: none;color:#b2b2b2;">密码由6-20位字母，数字和符号至少两种以上字符组合，区分大小写</div>

                                </div>
                            </div>
                            <div class="m-vf-item">
                                <label class="ui-label u-label">新密码：</label>
                                <div class="ui-form-item-group">
                                    <div class="ui-tooltips ui-tooltips-warning ui-tooltips-bottom-left-arrow">
                                        <div class="ui-tooltips-content">
                                            <p class="ui-tooltips-msg" >密码由6-20位字母，数字和符号至少两种以上字符组合，区分大小写</p>
                                        </div>
                                    </div>
                                    <input name="password" maxlength="20" id="J_password_newpwd" name="verification-new-pwd" class="ui-input u-verification-input" placeholder="由6-20位字母，数字和符号组合" type="password">
                                </div>
                            </div>
                            <div class="m-vf-item">
                                ...
                                <div class="ui-form-item-group z-ui-form-item-success">
                                    ...
                                    <input name="aginpassword"  maxlength="20" id="J_password_againpwd" name="verification-check-new-pwd" class="ui-input u-verification-input" placeholder="请再次输入上面的密码" type="password">
                                </div>
                            </div>
                            <div class="m-vf-item">
                                <a id="J_password_sure" href="javascript:;" role="button" class="ui-btn-medium ui-btn-primary ui-btn-loading">
                                    <span id="sub" class="ui-btn-loading-before">确定</span>
                                    <span class="ui-btn-loading-after"><i class="ii-loading-gray-16x16"></i></span>
                                </a>
                            </div>
                        </div>
                        <input type="hidden" id="userid" value="<?php echo $_smarty_tpl->tpl_vars['arr']->value['id'];?>
"/>
                        <!-- 原登录密码验证——步骤二 -->
                        <div style="display: none;" id="J_step" class="J_step f-hidden">
...
<script type="text/javascript">
    //检测两次密码输入是否正确
    var jiupassword = $("#J_password_oldpwd").val();//旧密码
    var newpwd = $("#J_password_newpwd").val();//新密码
    var againpwd = $("#J_password_againpwd").val();//再次输入密码
    if(newpwd!=againpwd)
    {
        $("#J_password_newpwd").css("border","1px solid #f20266");
        $("#J_password_againpwd").css("border","1px solid #f20266");
        $(".ui-tooltips-msg").show();
    }
    else{
        $("#J_password_newpwd").css("border","1px solid #b2b2b2");
        $("#J_password_againpwd").css("border","1px solid #b2b2b2");
        $(".ui-tooltips-msg").hide();
    }
    $("#J_password_oldpwd").focus(function(){
        $("#J_password_oldpwd_err").css("display","none");
        $("#J_password_oldpwd").css("border","1px solid #b2b2b2");
    });
    //提交修改
    $("#sub").click(function(){
        var jiupassword = $("#J_password_oldpwd").val();//旧密码
        var newpwd = $("#J_password_newpwd").val();//新密码
        var id = $("#userid").val();
        $.post("?c=user&a=resetoldpassword", {newpwd: newpwd,jiupassword:jiupassword,id:id}, function (msg) {
            if(msg==0)
            {
                $("#J_password_oldpwd_err").css("display","block");
                $("#J_password_oldpwd").css("border","1px solid #f20266");
            }
            if(msg==1){
                //修改成功，跳到登录页
