<?php /* Smarty version Smarty-3.1.14, created on 2017-03-29 08:53:57
         compiled from ".\template\User\user_bindemail.html" */ ?>
<?php /*%%SmartyHeaderCode:402758db6b11201394-80312564%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '2e26dd4c4f3f3ff351eb6c0901599fa18af11f83' => 
    array (
      0 => '.\\template\\User\\user_bindemail.html',
      1 => 1490777351,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '402758db6b11201394-80312564',
  'function' => 
  array (
  ),
  'version' => 'Smarty-3.1.14',
  'unifunc' => 'content_58db6b112f44b6_65599567',
  'variables' => 
  array (
    'arr' => 0,
  ),
...
<?php if ($_valid && !is_callable('content_58db6b112f44b6_65599567')) {function content_58db6b112f44b6_65599567($_smarty_tpl) {?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
            ...
            <div class="user_Account_style">
                ...
                <div id="J_main" class="m-content">
                    ...
                    <div class="m-content-hd">
                        <div class="m-page-tab"><a href="?c=user&a=reset" class="tab-item z-current" data-track="done">邮箱验证</a></div>
                    </div>
                    <!-- 手机验证——步骤一 -->
...
"/>
                            <div class="m-vf-item">
                                <label class="ui-label u-label">验证邮箱：</label>
                                <input type="text" id="email" class="ui-input u-verification-code" placeholder="输入验证邮箱"/>
                            </div>
                            <div class="m-vf-item">
                                ...
                                <div class="ui-form-item-group">
                                    ...
                                    <input maxlength="6" placeholder="邮箱验证码" class="ui-input u-verification-code" id="J_codeInput" name="verification-code" type="text">
                                    <!-- 点击前 -->
                                    <a  role="button" id="J_getCodeBtn" class="ui-btn-medium u-btn-get-code ui-btn-secondary">获取验证码</a>
                                    <!-- 点击后 -->
                                    <a style="display: none;" role="button" id="J_CodeBtn" class="ui-btn-medium ui-btn-disable u-btn-get-code ui-btn-secondary"><span class="second">58</span>S后重新获取</a>
                                </div>
                            </div>
                            <div class="m-vf-item">
...
<script type="text/javascript">
    ...
    $("#email").focus(function(){
        $("#email").css("border","1px solid #b2b2b2");
    });
    //发送邮箱验证码
    $("#J_getCodeBtn").click(function(){
        var email = $("#email").val(); //获取邮箱
        $.post("?c=user&a=checkemail", {email: email}, function (msg) {
            if(msg==1){
                $(".success").html("系统已向您的邮箱发送了一封邮件<br/>请登录到您的邮箱查看您的校验码及时重置您的密码！");
            }
        });
    });
    //提交验证码
    $("#sub").click(function(){
        ...
        var email = $("#email").val(); //获取邮箱
        var id = $("#userid").val();
        $.post("?c=user&a=checkemailyzm", {yzm: yzm,email:email,id:id}, function (msg) {
            if (msg == 0) {
