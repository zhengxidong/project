...
class GoodsController extends Controller{
    ...
    public function select(){
        ...
        $GLOBALS['smarty']->display("goods/select.html");
    }
    //搜索
    public function search(){
        ...
        {
            $coarr = $gs->select("select * from tb_goods where {$_POST['sjtwo']}<=sj and sj>={$_POST['sj']}");
            $GLOBALS['smarty']->assign("arr",$coarr);
            $GLOBALS['smarty']->display("goods/select.html");
        }
        //带时间区间、名称搜索
        if(!empty($_POST['sj'])&&!empty($_POST['sjtwo'])&&!empty($_POST['goodsname'])){
            $coarr = $gs->select("select * from tb_goods where {$_POST['sjtwo']}<=sj and sj>={$_POST['sj']} and goodsname='{$_POST['goodsname']}'");
            $GLOBALS['smarty']->assign("arr",$coarr);
            $GLOBALS['smarty']->display("goods/select.html");
        }
        //带名称搜索
        if(!empty($_POST['goodsname'])){
            $coarr = $gs->select("select * from tb_goods where goodsname='{$_POST['goodsname']}'");
            $GLOBALS['smarty']->assign("arr",$coarr);
            $GLOBALS['smarty']->display("goods/select.html");
        }
