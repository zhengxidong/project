...
<section class="container-fluid">
  ...
  <div class="row">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <div class="row">
        <form action="/Admin/Resources/update" method="post" autocomplete="off" draggable="false">
            <input type="hidden" name="id" value="<?php echo ($resources['id']); ?>"/>
            <div class="col-md-9">
                <h1 class="page-header">增加资源</h1>
                <div class="add-article-box">
                    <h2 class="add-article-box-title"><span>所属资源分类</span></h2>
                    <div class="add-article-box-content">
                        <select name="resourcestypeid" id="">
                            <?php if(is_array($resourcestype)): foreach($resourcestype as $key=>$value): if($resources['resourcestypeid'] == $value['id']): ?><option value="<?php echo ($value['id']); ?>" selected><?php echo ($value['resourcestypename']); ?></option>
                                    <?php else: ?>
                                    <option value="<?php echo ($value['id']); ?>"><?php echo ($value['resourcestypename']); ?></option><?php endif; endforeach; endif; ?>
                        </select>
                        <span class="prompt-text">用简洁的文字描述资源。</span> </div>
                </div>
                <div class="add-article-box">
                    <h2 class="add-article-box-title"><span>名称</span></h2>
                    <div class="add-article-box-content">
                        <input type="text" name="name" value="<?php echo ($resources['name']); ?>" class="form-control" placeholder="请输入资源名称" autocomplete="off">
                        <span class="prompt-text">用简洁的文字描述。</span> </div>
                </div>
                <div class="add-article-box">
                    <h2 class="add-article-box-title"><span>地址（URL）</span></h2>
                    <div class="add-article-box-content">
                        <input type="text" name="url" value="<?php echo ($resources['url']); ?>" class="form-control" placeholder="在此处输入资源地址（URL）" required autocomplete="off">
                    </div>
                </div>
                <div class="add-article-box">
                    <h2 class="add-article-box-title"><span>密码</span></h2>
                    <div class="add-article-box-content">
                        <input type="text" name="resourcespw" value="<?php echo ($resources['resourcespw']); ?>" class="form-control" placeholder="请输入资源名称" autocomplete="off">
                        <span class="prompt-text">用简洁的文字描述。</span> </div>
                </div>
            </div>
            <div class="col-md-3">
                ...
                <div class="add-article-box">
                    ...
                    <div class="add-article-box-content">
                        <p><label>公开度：</label>
                            <?php if($resources['ispublic'] == 0): ?><input type="radio" name="ispublic" value="0" checked/>公开
                                <input type="radio" name="ispublic" value="1" />加密
                                <?php else: ?>
                                <input type="radio" name="ispublic" value="0" />公开
                                <input type="radio" name="ispublic" value="1" checked/>加密<?php endif; ?>
                        </p>
                    </div>
